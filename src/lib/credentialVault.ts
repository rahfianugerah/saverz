import { db } from './db';
import type { CredentialEntry, CredentialVaultPayload, VaultRecord } from './types';

const CREDENTIAL_VAULT_ID = 'credential-vault';
...
async function encryptPayload(key: CryptoKey, payload: CredentialVaultPayload) {
...
async function decryptPayload(key: CryptoKey, ivBase64: string, ciphertextBase64: string): Promise<CredentialVaultPayload> {
  const iv = fromBase64(ivBase64);
  const ciphertext = fromBase64(ciphertextBase64);
  const decrypted = await crypto.subtle.decrypt({ name: 'AES-GCM', iv }, key, toStrictArrayBuffer(ciphertext));
  const json = new TextDecoder().decode(decrypted);
  const payload = JSON.parse(json) as CredentialVaultPayload;

  if (!payload || !Array.isArray(payload.entries)) {
    throw new Error('Credential vault payload format is invalid.');
  }

  return payload;
}

export async function getCredentialVaultRecord() {
  return db.vaults.get(CREDENTIAL_VAULT_ID);
}

export async function createCredentialVault(masterPassword: string) {
  const existing = await getCredentialVaultRecord();
  if (existing) {
    throw new Error('Credential vault already exists.');
  }
  ...
  const initialPayload: CredentialVaultPayload = {
    schemaVersion: 1,
    entries: [],
  };
...
export async function unlockCredentialVault(masterPassword: string) {
  const record = await getCredentialVaultRecord();
  if (!record) {
  ...
  try {
    ...
    throw new Error('Failed to unlock credential vault. Master password may be incorrect.');
  }
}

export async function saveCredentialEntries(key: CryptoKey, record: VaultRecord, entries: CredentialEntry[]) {
...
export async function resetCredentialVault() {
  await db.vaults.delete(CREDENTIAL_VAULT_ID);
}
